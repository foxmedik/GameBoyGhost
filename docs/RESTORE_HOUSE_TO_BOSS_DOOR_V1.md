...
Restore the published release Git commit into a new directory. First clone `https://github.com/Xe-Xo/LADXExperiments.git` into `references/LADXExperiments` and check out commit `23f09fcf0339c1497ef6778e69a33075dd716a9f`. Then copy the contents of the USB `essential-artifacts` package into the project clone, preserving relative paths. This order restores the private state file into the already-cloned runtime. The larger `workspace` backup retains all experiments, failures, data, and private media. The original qualified configs also exist at `runs/house-to-boss-door-v1-qualified-configs`; later research-status bookkeeping is distinct from the qualified controller.

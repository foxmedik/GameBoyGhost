...
def follow(env,rows,evidence):
    ...
    for index,segment in enumerate(segments):
        ...
        for tx,ty in targets:
            ...
            best_distance=999;no_progress=0;flank=[];flanked=False
            for _ in range(128):
                ...
                if distance<best_distance-2:best_distance=distance;no_progress=0
                else:no_progress+=1
                m=env.pyboy.memory
                moblin_clash=any(m[0xC280+i]==5 and m[0xC3A0+i]==0x14
                    and -48<=int(m[0xC200+i])-s['x']<=-16
                    and abs(int(m[0xC210+i])-s['y'])<=12 for i in range(16))
                if (segment['room']==[0,0,0x52] and no_progress>=24 and not flanked
                        and moblin_clash and s['y']>=100 and dx<0):
                    flank=[(s['x'],86),(tx,86)]
                    flanked=True
                    evidence.append(dict(kind='sword_moblin_lateral_detour',frame=env.frames,waypoints=flank.copy()))
                while flank and abs(flank[0][0]-s['x'])<=3 and abs(flank[0][1]-s['y'])<=3:
                    flank.pop(0)
                if flank:dx,dy=flank[0][0]-s['x'],flank[0][1]-s['y']
                elif abs(dx)<=3 and abs(dy)<=3:break

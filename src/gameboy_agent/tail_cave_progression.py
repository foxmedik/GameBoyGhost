...
def clear_compass_room(env, evidence, *, budget=1024):
    ...
    for decision in range(budget):
        ...
        if not targets:
            ...
            for _ in range(96):
                ...
                env.step_buttons([direction, 'b'] if direction and env.pyboy.memory[0xFF9E] != {'right':0,'left':1,'up':2,'down':3}[direction] else ['b'], action_frames=1)
        ...
        direction = projectile_direction(env, state)
        if direction:
            env.step_buttons([direction, 'b'] if env.pyboy.memory[0xFF9E] != {'right':0,'left':1,'up':2,'down':3}[direction] else ['b'], action_frames=1)
            guard_facing = direction
            continue
        guard_facing = None

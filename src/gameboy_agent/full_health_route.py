...
def clear_room0e_worm(env,evidence,budget=3600):
    r=SafeRouteSteps(env)
    ...
    for index in range(budget):
        ...
        if phase_route:
            ...
            if walk in ('left','right'):
                ...
                if target==18 and not 38<=s['x']<=42:
                    # Recoil can occur after the horizontal leg completed.
                    # Reacquire the x40 aisle throughout the northbound leg.
                    walk='right' if s['x']<40 else 'left'
            buttons=[walk,'b']

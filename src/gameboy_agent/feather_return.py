...
def return_to_room0e(env,evidence):
    ...
    r.move(7,'down',lambda s:s['y']>=50)
    # Keese activate strictly within32 pixels; y41 never wakes the bat at80.
    # Clear it from north of the trap before jumping.
    m=env.pyboy.memory
    last_swing=-32
    for index in range(600):
        ...
        s=r.check(7)
        if not all(m[0xC290+i]==0 and int(m[0xC200+i])>=96 and int(m[0xC210+i])>=92 for i in bats):
            raise RouteBlocker('North-side room07 Keese clearance budget exhausted')
        # A resting bat in the southeast cannot be awakened from this bank.
        # Cross on the west, outside its activation range, then defend below.
        r.move(7,'left',lambda s:s['x']<=56)
        r.move(7,'down',lambda s:s['y']>=82,jump=True)
        settle(env,7)
        from gameboy_agent.progression_skills import equip_item
        from gameboy_agent.dungeon_items import ItemSteps
        equip_item(env,4,button='b')
        ItemSteps(env).move(7,'right',lambda s:s['x']>=70,defend=True)
        ItemSteps(env).move(7,'down',lambda s:s['y']>=112,defend=True)
        ItemSteps(env).travel(7,'down',13)
        ...
        evidence.append(dict(kind='room07_resting_keese_west_bypass',frame=env.frames))
        return

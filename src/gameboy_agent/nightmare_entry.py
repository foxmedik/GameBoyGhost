...
def third_key_and_north_exit(env,evidence):
    ...
    if snapshot(env.pyboy)['x']>=84 and snapshot(env.pyboy)['y']>80:
        r.move(14,'up',lambda s:s['y']<=80)
    ...
    settle(env,14)
    # Landing can drift to74,59, outside the chest interaction position.
    if snapshot(env.pyboy)['x']>72:r.move(14,'left',lambda s:s['x']<=72,budget=8)
    if snapshot(env.pyboy)['y']>58:r.move(14,'up',lambda s:s['y']<=58,budget=8)

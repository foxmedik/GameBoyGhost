...
def armed_move(env, room, direction, done, budget=500, reactive_stalfos=False, reactive_enemies=False):
    r = SafeRouteSteps(env) if reactive_stalfos or reactive_enemies else RouteSteps(env)
    r.check(room)
    ...
    for index in range(budget):
        ...
        buttons=[direction] + (['a'] if strike else [])
        if reactive_enemies:
            buttons=[direction]
            close=[i for i in range(16) if m[0xC280+i]==5 and m[0xC3A0+i] in (25,30)
                   and abs(int(m[0xC200+i])-state['x'])<=24
                   and abs(int(m[0xC210+i])-int(m[0xC310+i])-state['y'])<=24]
            if close:
                i=min(close,key=lambda i:abs(int(m[0xC200+i])-state['x'])+abs(int(m[0xC210+i])-int(m[0xC310+i])-state['y']))
                dx=int(m[0xC200+i])-state['x'];dy=int(m[0xC210+i])-int(m[0xC310+i])-state['y']

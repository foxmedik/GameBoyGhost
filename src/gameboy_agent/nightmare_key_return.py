...
def cross_room10_with_stalfos(env):
    ...
    for index in range(60):
        ...
        if (0<m[0xFFA2]<=12 and m[0xFF9E]==0 and not m[0xC137]
                and any(m[0xC280+i]==5 and m[0xC3A0+i]==30
                        and 0<=int(m[0xC200+i])-s['x']<=28
                        and abs(int(m[0xC210+i])-int(m[0xC310+i])-(s['y']-int(m[0xFFA2])))<=24
                        for i in range(16))):
            buttons.append('a')
    ...
    armed_move(env,16,'up',lambda s:s['y']<=72,budget=96,reactive_enemies=True)

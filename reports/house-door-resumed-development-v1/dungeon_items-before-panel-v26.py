...
def spiked_beetles_to_stairs(env,evidence):
    ...
    for _ in range(900):
        ...
        facing=dict(right=0,left=1,up=2,down=3)[direction]
        buttons=['b']
        # Keep pursuit away from corner pits. Outlying flipped beetles can
        # recover and charge the shield on the central floor.
        tx=max(64,min(104,int(m[0xC200+i])))-s['x']
        ty=max(56,min(88,int(m[0xC210+i])))-s['y']
        # A beetle beyond a top corner cannot charge a diagonally offset
        # central guard. Approach its outer aisle via the clear middle row.
        outer_bait = not flipped and int(m[0xC210+i]) < 64 and (int(m[0xC200+i]) >= 120 or int(m[0xC200+i]) <= 40)
        if outer_bait:
            bait_x = 136 if int(m[0xC200+i]) >= 120 else 24
            tx = bait_x-s['x']
            ty = 72-s['y']
        ...
        if flipped and aligned and in_reach:
            ...
            if 64<=nx<=104 and 56<=ny<=88:walk=candidate
            elif m[0xFF9E]!=facing:walk=direction
        elif outer_bait and abs(ty)>2:
            walk='down' if ty>0 else 'up'
        elif outer_bait and abs(tx)>2:
            walk='right' if tx>0 else 'left'
        elif abs(dx)+abs(dy)>24 and abs(tx)+abs(ty)>2:

...
def clear_room0e_worm(env,evidence,budget=900):
    ...
    for index in range(budget):
        ...
        remote=(region=='lower' and m[0xC210+i]<64) or (region=='upper' and m[0xC210+i]>80)
        remote_frames=remote_frames+1 if remote and not phase_route else 0
        if remote_frames>=60:
            region='upper' if region=='lower' else 'lower'
            phase_route=([('up',80),('left',40),('up',18),('right',80)] if region=='upper'
                         else [('left',40),('down',80),('right',88)])
            remote_frames=0
        ...
        if region=='lower' and s['y']<80 and s['x']<40:
            ...
            target_y=18 if region=='upper' else max(80,min(108,int(m[0xC210+i])))
            target_x=(max(64,min(96,int(m[0xC200+i]))) if region=='upper'
            ...
            if abs(dx)>22 or abs(dy)>22:
                ...
                if s['x']<88 and target_y>84:walk='right'

...
def reach_rolling_bones(env):
    ...
    armed_move(env, 15, 'right', lambda s: s['x'] >= 135, reactive_stalfos=True)
    armed_move(env, 15, 'up', lambda s: s['y'] <= 72, reactive_stalfos=True)
...
def cross_room10_with_stalfos(env):
    ...
    for direction, done in [('up', lambda s: s['y'] <= 32),
        ...
        for _ in range(360):
            ...
            buttons=[direction] + (['a'] if near_stalfos else [])
            close=[i for i in range(16) if m[0xC280+i]==5 and m[0xC3A0+i]==30
                   and abs(int(m[0xC200+i])-s['x'])<=24
                   and abs(int(m[0xC210+i])-int(m[0xC310+i])-s['y'])<=24]
            if close:

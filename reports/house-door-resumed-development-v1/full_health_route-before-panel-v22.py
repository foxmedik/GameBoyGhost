...
def clear_room0d_worm(env, evidence, budget=600):
    ...
    for index in range(budget):
        ...
        if abs(dx)+abs(dy)>40:
            target_y=max(32,min(80,int(m[0xC210+i])))
            walk=('right' if state['x']<68 else 'left' if state['x']>76
                  else 'down' if state['y']<target_y-2 else 'up' if state['y']>target_y+2 else None)

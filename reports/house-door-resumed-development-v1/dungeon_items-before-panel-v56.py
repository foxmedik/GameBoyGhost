...
def compass_and_second_key(env, evidence):
    ...
    if snapshot(env.pyboy)['y'] < 88:
        r.move(21, 'down', lambda s: s['y'] >= 88)
    else:
        r.move(21, 'up', lambda s: s['y'] <= 88)
...
def clear_map_enemies(env, evidence, budget=600, room=20, kinds=(25, 30)):
    ...
    for index in range(budget):
        ...
        if room==20:
            # The middle west aisle reaches doorway-side enemies; outside
            # that band retain the inset guard against the corner pits.
            west_limit=20 if 56<=s['y']<=72 else 40
            if s['x']<=west_limit:
                if 'left' in buttons:buttons.remove('left')
                if s['x']<west_limit:buttons=['right','b']
            if s['x']<=40 and m[0xC200+i]<24 and 56<=m[0xC210+i]<=88 and not 56<=s['y']<=72:
                buttons=['down' if s['y']<56 else 'up']
...
def reach_south_worm_entry(env, evidence):
    ...
    # Bat pursuit can finish far from the entrance lane. The upper row is
    # crumbling floor except at x64..95; rejoin it from the solid row below.
    r.move(18,'up' if snapshot(env.pyboy)['y']>43 else 'down',lambda s:42<=s['y']<=44)
    r.move(18,'right' if snapshot(env.pyboy)['x']<80 else 'left',lambda s:78<=s['x']<=82)
    r.travel(18, 'up', 13)
...
def detour_room05(env,evidence):
    ...
    if any(env.pyboy.memory[0xC280+i] and env.pyboy.memory[0xC3A0+i]==28 for i in range(16)):
        r.move(5,'left',lambda s:s['x']<=104,defend=True)
        m=env.pyboy.memory
        last=-32
        for _ in range(400):
            s=r.ready(5)
            gels=[i for i in range(16) if m[0xC280+i] and m[0xC3A0+i]==28]
            if not gels:break
            targets=[i for i in gels if m[0xC280+i]==5]
            keys=['b']
            if targets:
                i=min(targets,key=lambda i:abs(int(m[0xC200+i])-s['x'])+abs(int(m[0xC210+i])-s['y']))
                ...
                if m[0xC290+i]==4:
                    # Clinging Gels ignore sword damage; held input hastens
                    # release. Stay clear of the Spark before crossing.
                    keys=['b']
                elif not m[0xC137]:
                    if abs(dx)>18 or abs(dy)>18:
                        # Recoil may displace the guard. Reach the left Gel
                        # from above its block, using the clear top row to
                        # avoid the x104/y40 block and right-hand torch.
                        walk=('left' if s['x']>122 else 'up' if s['x']>92 and s['y']>29
                              else 'left' if s['x']>90 else 'right' if s['x']<86
                              else 'down' if s['y']<38 else 'up' if s['y']>42 else None)
                        if walk:keys.append(walk)
                    elif m[0xFF9E]!=face:keys.append(aim)
                    elif abs(dx)<=18 and abs(dy)<=18 and env.frames-last>=24:
                        keys.append('a');last=env.frames
            env.step_buttons(keys,action_frames=1)
        else:raise RouteBlocker('Room05 upper Gel clearance budget exhausted')
        if snapshot(env.pyboy)['x']>122:
            r.move(5,'left',lambda s:s['x']<=120)
        r.move(5,'up',lambda s:s['y']<=27)
        if snapshot(env.pyboy)['x']<88:
            r.move(5,'right',lambda s:s['x']>=88)
...
def spiked_beetles_to_stairs(env,evidence):
    ...
    for _ in range(900):
        ...
        if not targets:
            env.step_buttons([],action_frames=1)
            continue
        upright=[i for i in targets if m[0xC290+i]!=3
                 and abs(int(m[0xC200+i])-s['x'])<=36 and abs(int(m[0xC210+i])-s['y'])<=36]
        # Guard approaching upright beetles before committing to a sword
        # animation against a flipped one on the opposite flank.
        i=min(upright or targets,key=lambda i:abs(int(m[0xC200+i])-s['x'])+abs(int(m[0xC210+i])-s['y']))
        ...
        elif 64<=ex<=104 and ey>=80:
            # The central floor reaches y88; this stance puts a flipped
            # bottom-edge target within sword reach without entering a pit.
            gx,gy=ex,min(88,ey)
        else:
        ...
        if (gx<64 or gx>104) and not 64<=s['y']<=80:
            ...
            gx,gy=max(64,min(104,s['x'])),72
        tx,ty=gx-s['x'],gy-s['y']
        aligned=abs(dy)<=12 if direction in ('left','right') else abs(dx)<=12
        ...
        if flipped and aligned and in_reach:
            if m[0xFF9E]!=facing:walk=direction
                ...
                buttons.append('a');last_swing=env.frames
        elif not ((64<=s['x']<=104 and 24<=s['y']<=88) or (24<=s['x']<=136 and 64<=s['y']<=80)):
            # Shield recoil can leave the pursuit lanes. Return vertically
            # through the outside aisle before attempting lateral alignment.
            walk='up' if s['y']>80 else 'down' if s['y']<64 else 'right' if s['x']<24 else 'left'
...
def reclear_hardhat_return(env,evidence):
    ...
    for _ in range(1800):
        ...
        if not targets:
            ...
            return
        active=[t for t in targets if t['status']==5]
        near=[t for t in active if abs(t['x']-s['x'])+abs(t['y']-s['y'])<=32]
        m=env.pyboy.memory
        if near:
            # Do not swing at a dying beetle while a live one approaches from
            # another direction. Observe actual facing after sword lock ends.
            teacher.phase=None
            target=min(near,key=lambda t:abs(t['x']-s['x'])+abs(t['y']-s['y']))
            dx,dy=target['x']-s['x'],target['y']-s['y']
            aim=('right' if dx>0 else 'left') if abs(dx)>abs(dy) else ('down' if dy>0 else 'up')
            face={'right':0,'left':1,'up':2,'down':3}[aim]
            buttons=['b'] if m[0xC137] else [aim,'b'] if m[0xFF9E]!=face else ['a','b']
            frames=1
        elif active:
            teacher.phase=None
            buttons,frames=teacher.action(s,active)
        else:
            buttons,frames=['b'],1

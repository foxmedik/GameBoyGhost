...
def spiked_beetles_to_stairs(env,evidence):
    ...
    for _ in range(900):
        ...
        tx=max(64,min(112,int(m[0xC200+i])))-s['x']
        ty=max(48,min(96,int(m[0xC210+i])))-s['y']
        aligned=abs(dy)<=8 if direction in ('left','right') else abs(dx)<=8
        walk=None
        if flipped and abs(dx)<=24 and abs(dy)<=24 and not aligned:
            walk=('down' if dy>0 else 'up') if direction in ('left','right') else ('right' if dx>0 else 'left')
        ...
        if (flipped and aligned and abs(dx)<=24 and abs(dy)<=24 and m[0xFF9E]==facing
                and not m[0xC137] and env.frames-last_swing>=24):
            buttons.append('a');last_swing=env.frames
        env.step_buttons(buttons,action_frames=1)

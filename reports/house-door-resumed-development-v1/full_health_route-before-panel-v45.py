...
def clear_room0d_worm(env, evidence, budget=600):
    ...
    for index in range(budget):
        ...
        if remote>=80 and not crossing:
            ...
            new_guard=120 if ex>=112 and ey<=64 else 24 if ex<=40 and ey<=64 else 72
        ...
        if crossing or abs(dx)+abs(dy)>40:
            target_y=max(32,min(64 if guard_x!=72 else 80,int(m[0xC210+i])))
            # Row64 connects the three aisles below the paired statues and
            # above the lower side pits. Do not cut diagonally through them.

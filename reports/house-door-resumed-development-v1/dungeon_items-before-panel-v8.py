...
class ItemSteps:
    ...
    def move(self, room, direction, done, budget=180, defend=False):
        ...
        for _ in range(budget):
            ...
            if targets and self.env.frames-self.last_swing >= 24:
...
def compass_and_second_key(env, evidence):
    ...
    r.move(19, 'right', lambda s: s['x'] >= 88, defend=True)
...
def clear_map_enemies(env, evidence, budget=600, room=20, kinds=(25, 30)):
    ...
    for index in range(budget):
        ...
        if stagnant>=12 and room==20:
            ...
            lane=32 if m[0xC200+i]<64 else (120 if m[0xC200+i]>96 else 80)
            target_y=max(32,min(110,int(m[0xC210+i])))
            entry_lane=min((32,80,120),key=lambda v:abs(v-s['x']))
        ...
        while detour:
            ...
            value=s['y'] if direction=='down' else s['x']
            done=value<=target if direction=='left' else value>=target
...
def spiked_beetles_to_stairs(env,evidence):
    ...
    for _ in range(900):
        ...
        facing=dict(right=0,left=1,up=2,down=3)[direction]
        buttons=['b']
        if abs(dx)+abs(dy)>24 or m[0xFF9E]!=facing:buttons.append(direction)
        if flipped and abs(dx)<=24 and abs(dy)<=24 and m[0xFF9E]==facing and env.frames-last_swing>=24:
            buttons.append('a');last_swing=env.frames
        env.step_buttons(buttons,action_frames=1)

...
OUT=ROOT/'runs/navigation-sword-v2'
...
def main():
 ...
 assert sha256(ROOT/'scripts/train_navigation_buttons.py')==plan['trainer_sha256']
 ...
 for p,h in plan['baseline_hashes'].items():assert sha256(ROOT/p)==h
 (OUT/'result.json').write_text(json.dumps(report,indent=2));(ROOT/'reports/navigation-sword-v2.json').write_text(json.dumps(report,indent=2));print('FINAL_GATES',json.dumps(gates),flush=True)

 ...
 for item in json.loads((teacher/'summary.json').read_text())['results']:
  ...
  try:
   ...
   for n in range(128):
    ...
    x=torch.from_numpy((temporal_feature(senses(env),history)-saved['mean'])/saved['scale']);a=int(m(x).argmax());buttons,frames=action_command(a)
...
if __name__=='__main__':
 p=ROOT/'configs/room15_student_v5_recovery_skill.json';o=ROOT/'runs/room15-v5-recovery-skill-gate';o.mkdir();run(json.loads(p.read_text())|{'candidate':'runs/room15-v5-recovery-skill-candidate/candidate.pt'},o)

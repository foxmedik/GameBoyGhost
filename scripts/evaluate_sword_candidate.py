...
def main():
 ...
 for p,h in plan['baseline_hashes'].items():assert sha256(ROOT/p)==h
 (OUT/'result.json').write_text(json.dumps(report,indent=2));(ROOT/'reports/navigation-sword-v1.json').write_text(json.dumps(report,indent=2));print('FINAL_GATES',json.dumps(gates),flush=True)

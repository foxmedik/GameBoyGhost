...
def run_case(root,spec,plan):
 ...
 env=make();rows=[];evidence=[];history=[];recovery=EarlyY80Recovery();failure=None;started=time.monotonic()
 try:
  ...
  for _ in range(plan['decision_budget']):
   ...
   buttons,guided=recovery.action(state)
   if guided:
    action=1;frames=1;evidence.append(dict(kind='guided_early_y80_recovery',frame=env.frames,x=state['x'],y=state['y'],intervention=recovery.interventions))
   else:
 ...
 result=dict(spec=spec,success=failure is None,failure=failure,initial=initial,final=final,damage_raw=damage,exact_replay=True,student_decisions=len(history),control='v4_student_with_state_checked_guided_early_y80_recovery',guided_interventions=recovery.interventions,teacher_action_fraction=0.0,autonomous_room15_evaluated=False,seconds=round(time.monotonic()-started,2));write(out/'evidence.json',evidence);write(out/'result.json',result);write(out/'manifest.json',{p.name:file_hash(p) for p in out.iterdir() if p.is_file()});return result
def main():

...
def run(shield=False):
    source=ROOT/'runs/progression-attempt-v2';out=ROOT/('runs/progression-west-passage-shield-v1' if shield else 'runs/progression-west-passage-v2')
    out.mkdir(exist_ok=False)
    for name in ('initial.state','game.gbc'):shutil.copy2(source/name,out/name)
    prefix=[json.loads(line) for line in (source/'trajectory.jsonl').read_text().splitlines()][:736]
    write(out/'plan.json',dict(source_trace_hash=file_hash(source/'trajectory.jsonl'),prefix_decisions=736,shield_held=shield,
    ...
    for replay in (False,True):
        ...
        try:
            ...
            for _ in range(40):
                ...
                _,_,done,truncated,info=env.step([movement,2 if shield else 0]);after=snapshot(env.pyboy)
    ...
    write(ROOT/'reports'/f'{out.name}.json',dict(result=result,output=str(out.relative_to(ROOT)),
        memory_version=memory.version,source_hash=file_hash(Path(__file__))))
    print(json.dumps(result,indent=2))


if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--shield',action='store_true');run(p.parse_args().shield)

...
def inspect(key):
 ...
 dest=ROOT/'runs/proximity-sword-v2-diagnosis';dest.mkdir(exist_ok=True)
  ...
  with tempfile.TemporaryDirectory() as tmp:
   ...
   try:
    ...
    for i,action in enumerate(r['actions']):
     info=inputs(base)
     records.append(dict(step=i,state=senses(base).__dict__,inputs=info,action=action,velocities=[dict(slot=e['slot'],vx=int(base.pyboy.memory[0xC240+e['slot']]),vy=int(base.pyboy.memory[0xC250+e['slot']])) for e in info['entities']]))
     if i==first:
   ...
   report[variant]['trace']=records
   report[variant]['damage_events']=[e for e in r['events'] if e['damage']]

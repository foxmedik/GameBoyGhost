...
from gameboy_agent.room15_model import action_command
...
def run_case(root,spec,plan):
 ...
 try:
  ...
  for _ in range(plan['decision_budget']):
   ...
   buttons,frames=action_command(action);traced.step_buttons(buttons,action_frames=frames);history=update(history,action,state,normalized(snapshot(env.pyboy)))
 ...
 result=dict(spec=spec,success=failure is None,failure=failure,initial=initial,final=final,damage_raw=damage,exact_replay=True,student_decisions=len(history),control='student_only_room15_temporal',teacher_action_fraction=0.0,autonomous_room15_evaluated=True,seconds=round(time.monotonic()-started,2));write(out/'evidence.json',evidence);write(out/'result.json',result);write(out/'manifest.json',{p.name:file_hash(p) for p in out.iterdir() if p.is_file()});return result
def main():

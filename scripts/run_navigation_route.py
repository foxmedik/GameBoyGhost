"""Fixed sword controller followed by an ordered navigation route."""
import argparse
import json
...
def source_hashes():
    ...
    paths += [ROOT/'scripts'/name for name in ('run_navigation_route.py','route_progress.py','control_context.py','tree_controller.py','sword_imitation.py','run_skill_chain.py')]
    return {str(p.relative_to(ROOT)):sha256(p) for p in paths}


def run(out,checkpoint=None,*,start='house',route=None,budget=256,stop_after=None,resume=None):
    ...
    if resume:
        ...
        route,start,budget=saved['route'],saved['start'],saved['budget']
    ...
    if budget<1 or not route or not route['goals']:raise ValueError('A route and positive budget are required')
    goals=route['goals']
    ...
    base=TrainingEnv(out/'game.gbc',out/'initial.state',max_steps=1600+budget*len(goals)+1,sword_curriculum=False)
    ...
    progress=dict(cursor=0,leg_steps=0,leg_start=None,completed=[])
    try:
        ...
        if saved:
            ...
            damage=saved['damage'];nav_damage=saved['navigation_damage'];progress=saved['progress']
        with (out/'trajectory.jsonl').open('x') as log:
            while stop_after is None or len(actions)<stop_after:
                ...
                if state.sword:
                    if sword_step is None:sword_step=len(actions);progress['leg_start']=len(actions)
                    outcome=advance(state,goals,progress,len(actions),budget)
                    if outcome!='running':status=outcome;break
                    goal=goals[progress['cursor']]
                    action=policy.action(obs,state.room,goal);skill='navigation';nav_steps+=1;progress['leg_steps']+=1
                else:
                ...
                log.write(json.dumps(dict(step=len(actions)-1,skill=skill,action=action,room=list(after.room),
                    x=after.x,y=after.y,health=after.health,reward=reward,goal=goals[progress['cursor']] if progress['cursor']<len(goals) else None,goal_index=progress['cursor']))+'\n')
        ...
        result=dict(run_id=str(uuid.uuid4()),episode_id=saved['episode_id'] if saved else str(uuid.uuid4()),
            parent_checkpoint=str(resume) if resume else None,start=start,route=route,budget=budget,progress=progress,
            ...
            goal_source='scripted ordered route; no general planner active')
        ...
        print(json.dumps({k:result[k] for k in ('start','status','steps','sword_step','navigation_steps','navigation_damage','progress')}),flush=True)
        return result
    finally:env.close()
...
if __name__=='__main__':
    ...
    p.add_argument('--start',choices=list(STARTS),default='house');p.add_argument('--route-id')
    p.add_argument('--curriculum',type=Path,default=ROOT/'configs/navigation_routes_v1.json')
    p.add_argument('--stop-after',type=int);p.add_argument('--resume',type=Path)
    a=p.parse_args();route=None;budget=256
    if not a.resume:
        if not a.checkpoint or not a.route_id:p.error('--checkpoint and --route-id required')
        config=json.loads(a.curriculum.read_text());route=next(r for r in config['routes'] if r['id']==a.route_id);budget=config['budget_per_goal']
    run(a.out,a.checkpoint,start=a.start,route=route,budget=budget,stop_after=a.stop_after,resume=a.resume)

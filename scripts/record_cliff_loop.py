"""Render a verified continuous teacher loop at native frame speed to MP4."""
...
def main():
 out=ROOT/'runs/videos/cliff-loop-demonstration';out.mkdir(parents=True,exist_ok=False)
 manifest=ROOT/'runs/navigation-cliff-returns-v3/cliff-return_west-house-0/manifest.json';m=json.loads(manifest.read_text());assert m['replay_verified']
 demo=min(m['paths'],key=lambda p:len(p['actions']));run=Path(m['source_run']);r=json.loads((run/'result.json').read_text());assert sha256(run/'result.json')==m['source_result_sha256']
 actions=demo['prefix']+demo['actions'];goals=r['route']['goals'];cursor=0;frames=0;damage=0;milestones=[];recording=False
 ...
 with tempfile.TemporaryDirectory() as tmp:
  ...
  def emit(complete=False):
   ...
   draw.text((16,42),'Replay-verified demonstration • House start',font=small,fill='#9ed9c7')
  ...
  try:
   ...
   for index,a in enumerate(actions):
    ...
    if index+1==len(demo['prefix']):assert fingerprint(base)==demo['initial']
   assert cursor==4 and damage==0 and fingerprint(base)==demo['final']
   gameplay_frames=frames
   for _ in range(120):emit(complete=True)
   result=dict(video=str(video),source_manifest=str(manifest),source_manifest_sha256=sha256(manifest),teacher_demonstration=True,learned_policy_rollout=False,continuous_episode=True,

"""Fixed final-checkpoint goal-specialist preservation evaluation against v7."""
...
def main():
 plan=READ(OUT/'plan.json');model=OUT/'model/epoch-256.pt';parent=Path(plan['parent'])
 ...
 assert sha256(ROOT/'scripts/train_cliff_correction_v2.py')==plan['trainer_sha256']
 ...
 previous=READ(ROOT/'reports/navigation-cliff-specialists-v1.json');assert sha256(ROOT/'reports/navigation-cliff-specialists-v1.json')==plan['specialist_v1_result_sha256']
 previous_routes={(r['route'],r['start']):r for r in previous['routes']}
 specialist_preserved=all((previous_routes[(r['route'],r['start'])]['status']!='success' or r['status']=='success') and r['waypoints']>=previous_routes[(r['route'],r['start'])]['waypoints'] for r in routes)
 resumed=[]
 for label,script,folder,args in [('route_cliff','scripts/run_navigation_route.py',OUT/'candidate/room_loop-house',['--route-id','room_loop','--curriculum',OUT/'curriculum.json']),('route','scripts/run_navigation_route.py',OUT/'candidate/beach_return-house',['--route-id','beach_return','--curriculum',OUT/'curriculum.json']),('chain','scripts/run_navigation_chain.py',OUT/'chain-house',['--goal','0','0','226','36','121'])]:
  full=READ(folder/'result.json')
  stop=min(full['steps']-1,(full['progress']['completed'][1 if label=='route_cliff' else 0]['step']+2 if label.startswith('route') and full['progress']['completed'] else full['sword_step']+2))
  ...
  r=READ(dest/'result.json');fields=['actions','fingerprint','steps','sword_step','navigation_steps','damage','navigation_damage','status','final_state']+(['progress'] if label.startswith('route') else [])
 ...
 for p,h in plan['baseline_hashes'].items():assert sha256(ROOT/p)==h
 (OUT/'result.json').write_text(json.dumps(report,indent=2));(ROOT/'reports/navigation-cliff-specialists-v2.json').write_text(json.dumps(report,indent=2));print('FINAL_GATES',json.dumps(gates),flush=True)

"""Focused corrections plus explicit successful route anchors, all replay verified."""
import json
...
OUT=ROOT/'runs/navigation-routes-v3';PARENT=ROOT/'runs/navigation-routes-v2/model/epoch-008.pt'
...
def main():
 ...
 for name,key,panel,goodroot in [('damage','2b70007c69c8e7a7faa9399a6eea7e76e799e097862a30690fafdf1f90fa4f51','runs/navigation-cache-v2','runs/navigation-focused-v4/eval-original'),('fresh','d5f00d7e9f023bd1bb87305aefd12456734ef575a04390ba3dba2e409ee972bf','runs/navigation-live-correction-v3/fresh-panel','runs/navigation-routes-v2/eval-fresh')]:
  ...
  if name=='damage':
   bad=read(ROOT/f'runs/navigation-routes-v2/eval-original/{key}-learned.json');first=next(i for i,(x,y) in enumerate(zip(good['actions'],bad['actions'])) if x!=y)
   diagnosis.append(dict(name=name,case=key,first_divergence=first))
   for pos in [max(0,first-3),first,first+3]:add({**c,'segment_id':f'{name}-repair-{pos}','replay_extra':bad['actions'][:pos],'attempt_cap':32,'success_cap':3})
 # Preserve every waypoint of all three known successful beach routes explicitly.
 for start,version in [('house','v2'),('beach','v1'),('approach','v2')]:
 ...
 for route,start in [('beach_return','beach'),('west_return','beach'),('west_return','approach')]:
  chain=f'runs/navigation-routes-v2/candidate/{route}-{start}';r=read(ROOT/chain/'result.json');rows=[json.loads(l) for l in (ROOT/chain/'trajectory.jsonl').read_text().splitlines()]
  i=r['progress']['cursor'];begin=r['progress']['leg_start'];health=rows[begin-1]['health'];harm=next((a['step'] for a in rows[begin:] if a['health']<health),None)
  positions=sorted({begin,begin+4,(harm-3 if harm is not None else begin+24)})
 ...
 old=ROOT/'runs/navigation-routes-v2';curriculum=read(old/'curriculum.json')
 plan=dict(parent=str(PARENT),parent_sha256=sha256(PARENT),curriculum=curriculum,epochs=8,learning_rate=5e-6,retention_weight=8,retention_route_data=str(old/'data'),
  balanced_path_groups=['damage-','fresh-','beach-','west-'],preservation_summary=str(ROOT/'runs/navigation-routes-v1/candidate/summary.json'),
  preservation_summaries=[str(ROOT/f'runs/navigation-routes-{v}/candidate/summary.json') for v in ['v1','v2']],retired_cohorts=sorted(retired),diagnosis=diagnosis,jobs=[j[0] for j in jobs],

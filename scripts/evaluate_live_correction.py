"""Evaluate one live-trajectory correction candidate on three panels."""
import argparse
...
def evaluate(out):
    out=Path(out).resolve();model=out/'model/epoch-012.pt';plan=json.loads((out/'plan.json').read_text())
    identity=json.loads(model.with_suffix('.json').read_text())
    assert sha256(model)==identity['sha256'] and identity['identity']['source_sha256']==plan['collector_sha256']
    def run(args,log):
    ...
    panels=[('original',ROOT/'runs/navigation-cache-v2',ROOT/'runs/navigation-live-v2',ROOT/'runs/navigation-live-no-history-v1/result.json'),
            ...
            ('fresh',out/'fresh-panel',out/'fresh-parent',out/'fresh-parent/result.json')]
    def panel(spec):
        ...
        result=compare(str(parent),str(dest/'result.json'));result['summary']=json.loads((dest/'result.json').read_text())['summary']['learned']
    ...
    eligible=all(not r['lost'] and not r['corrected_damage'] and not r['corrected_deaths'] for r in results.values())
    ...
    report=dict(panels=results,continuous=continuous,eligible=eligible,policy_sha256=sha256(model),
        interpretation='Old panels now substantially training regressions; fresh panel excludes retired cohorts and both old live panels but remains development evaluation.',

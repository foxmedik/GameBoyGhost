"""Evaluate a frozen Tail Cave candidate without teacher combat recovery."""
...
PLAN=ROOT/'configs/tail_cave_dagger_v2_live.json'
...
def run_case(root,spec,plan,model,saved):
 ...
 with (out/'trajectory.jsonl').open('x') as stream:
  ...
  try:
   ...
   enter_beetle_room(env,evidence);context=ControlContext(base)
 ...
 result={'spec':spec,'success':failure is None,'failure':failure,'initial':initial,'final':final,'final_small_keys':final_keys,'candidate_decisions':decisions,'damage_raw':initial['health']-final['health'],'exact_replay':True,'evidence':evidence,'seconds':round(time.monotonic()-began,3)}

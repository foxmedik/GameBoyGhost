...
def learned_downstream_recovery(checkpoint,base):
    ...
    def recovery(env,evidence,destination):
        ...
        for _ in range(512):
            ...
            if teacher_mode:
                m=base.pyboy.memory
                close=[i for i in range(16) if m[0xC280+i]==5 and m[0xC3A0+i] in (0x09,0x0B,0x14,0x1B,0x1C,0xC5)
                       and abs(int(m[0xC200+i])-state['x'])<=24
                       and abs(int(m[0xC210+i])-int(m[0xC310+i])-state['y'])<=24]
                if close:
                    i=min(close,key=lambda i:abs(int(m[0xC200+i])-state['x'])+abs(int(m[0xC210+i])-state['y']))
                    ...
                    env.step_buttons(keys,action_frames=1);decisions+=1
                    continue
